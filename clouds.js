   // google.setOnLoadCallback(drawChart);
		...
		if (geoJSON.features.length > 0) {
			...
			for (var i = 0; i < geoJSON.features.length; i++) {
				...
				//if(geoJSON.features[i].dt != null){
					...
					console.log(geoJSON.features[i].day, geoJSON.features[i].clouds);
					if( geoJSON.features[i].clouds > 0 ){
						setGraphData.features.push([geoJSON.features[i].day, geoJSON.features[i].clouds]);
    ...
    function drawChart4() {
		...
		data4.addColumn('number','Clouds');
		data4.addRows(setGraphData.features);
		console.log(setGraphData.features);
        var options4 = {
          title: 'Cloudiness %',
          curveType: 'function',
        ...
        var chart4 = new google.visualization.LineChart(document.getElementById('curve_chart_clouds'));
