   // google.setOnLoadCallback(drawChart);
	...
	function getWeatherFromServer() {
		var requestString = "http://52.33.113.43:8080/FinalProject281Server/graph/zipcode/95112";
		...
		if (geoJSON.features.length > 0) {
			...
			for (var i = 0; i < geoJSON.features.length; i++) {
				...
				//if(geoJSON.features[i].dt != null){
					...
					console.log(geoJSON.features[i].day, geoJSON.features[i].wind_speed);
					if( geoJSON.features[i].wind_speed > 0 ){
						setGraphData.features.push([geoJSON.features[i].day, geoJSON.features[i].wind_speed]);
    ...
    function drawChart() {
		...
		data.addRows(setGraphData.features);
		console.log(setGraphData.features);
        var options = {
          title: 'Wind Speed in Meter/Second',
          curveType: 'function',
          legend: { position: 'bottom' }
        };

        var chart = new google.visualization.LineChart(document.getElementById('curve_chart_wind'));
