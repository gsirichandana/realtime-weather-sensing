// google.setOnLoadCallback(drawChart);
		...
		if (geoJSON.features.length > 0) {
			...
			for (var i = 0; i < geoJSON.features.length; i++) {
				...
				//if(geoJSON.features[i].dt != null){
					...
					console.log(geoJSON.features[i].day, geoJSON.features[i].temp);
					if( geoJSON.features[i].humidity > 0  ){
						setGraphData.features.push([geoJSON.features[i].day, geoJSON.features[i].humidity]);
    ...
    function drawChart2() {
		...
		data2.addRows(setGraphData.features);
		console.log(setGraphData.features);
        var options2 = {
          title: 'Humidity in %',
          curveType: 'function',
        ...
        var chart2 = new google.visualization.LineChart(document.getElementById('curve_chart_humidity'));
